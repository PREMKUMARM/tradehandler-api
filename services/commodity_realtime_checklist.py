...
def enforce_step_dependencies(
    ...
    while changed:
        ...
        for i, st in list(by_idx.items()):
            if not st.server_ok:
                continue
            deps = COMMODITY_STEP_DEPENDS_ON[i] if i < len(COMMODITY_STEP_DEPENDS_ON) else []
...
def _intraday_context(kite, token: int) -> Dict[str, Any]:
    """BB/EMA from Kite historical + ticker gap-fill (same engine as Nifty)."""
...
def _build_checklist_context(
    ...
    if spot > 0:
        ...
        if trade_plan:
            ...
            from services.commodity_trade_service import allow_offhours_commodity_place
