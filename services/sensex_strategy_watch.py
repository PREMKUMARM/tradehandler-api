...
Autonomous is blocked only by kill-switch env/file or SENSEX_WATCH_AUTONOMOUS=0.
"""
...
_MAX_EVENTS = 40
_DEFAULT_POLL_SEC = 5
_STATE_PATH = Path(os.getenv("SENSEX_WATCH_STATE_FILE", "data/sensex_strategy_watch.json"))
_lock = RLock()
...
def watch_autonomous_globally_disabled() -> bool:
    ...
    return _env_bool("SENSEX_WATCH_DISABLE_AUTONOMOUS", False)
...
def _poll_interval() -> float:
    try:
        sec = float(os.getenv("SENSEX_WATCH_POLL_SECONDS", str(_DEFAULT_POLL_SEC)))
...
def _read_persisted() -> Dict[str, Any]:
    ...
    try:
        ...
        log_warning(f"[SensexWatch] state read failed: {exc}")
        return _default_persisted()
...
def _write_persisted(data: Dict[str, Any]) -> None:
    try:
        ...
        log_error(f"[SensexWatch] state write failed: {exc}")
...
class V2StrategyWatch:
    ...
    def _kill_switch_active() -> bool:
        try:
            ...
            return is_kill_switch_active("sensex")
        except Exception:
            return True
    ...
    def _reset_day_if_needed(self) -> None:
        ...
        if self._session_date != today:
            ...
            if self._session_date is not None:
                emit_gate_audit_day_summary("sensex")
    ...
    def _max_trades_per_day(self) -> int:
        ...
        if self._last_paper_mode:
            try:
                ...
                return 30
        try:
            raw = os.getenv("SENSEX_WATCH_MAX_TRADES_PER_DAY", "").strip()
            if raw:
    ...
    def _setup_invalidated(self, plan: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            from services.sensex_order_guard import min_entry_confirmation_score
    ...
    async def _abort_stale_fill(
        ...
        *,
        ...
        try:
            ...
            log_warning(
                f"[SensexWatch] stale fill abort entry={entry_id} sym={sym} reason={reason} exit={exit_id}"
            )
        finally:
            with _lock:
                self._gtt_attach_in_progress = False
        ...
        try:
            ...
            if entry_id:
                ...
                if is_filled_order_status(st):
                    log_warning(
                        f"[SensexWatch] skip cancel — entry {entry_id} already {st}; use stale abort"
                    )
    ...
    def _skip_log_interval_sec(self) -> float:
        try:
            sec = float(os.getenv("SENSEX_WATCH_SKIP_LOG_SECONDS", "90") or 90)
    ...
    def _record_autonomous_skip(
        ...
        if should_emit:
            log_info(f"[SensexWatch] auto_skipped {sym} — {msg}")
            self._push_event(
                WatchEvent(
                    at=now.isoformat(),
                    kind="auto_skipped",
    ...
    def arm(
        ...
        **flat_run_fields: Any,
        ...
        if mode == "autonomous" and watch_autonomous_globally_disabled():
            raise ValueError(
                "Autonomous trading is disabled on this server (SENSEX_WATCH_DISABLE_AUTONOMOUS)"
            )
        ...
        log_info(f"[SensexWatch] Armed mode={mode} autonomous={auto_place}")
    ...
    def reset_placement_counters(self) -> None:
        ...
        with _lock:
            ...
            self._signal_fired_today = False
        self._persist()
        log_info("[SensexWatch] Placement counters reset")
    ...
    def nuclear_reset(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log_error(f"[SensexWatch] nuclear reset unlink failed: {exc}")
        ...
        log_info("[SensexWatch] Nuclear reset")
        return self.status()
    ...
    def status(self) -> Dict[str, Any]:
        ...
        with _lock:
            ...
            cfg = self._cfg
            min_score = int(os.getenv("SENSEX_AUTO_MIN_ENTRY_SCORE", "65") or 65)
    ...
    def register_loop(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        with _lock:
            armed = self._armed
        if armed:
            log_info("[SensexWatch] Restored armed watch from disk — starting loop")
    ...
    async def _run_loop(self) -> None:
        while True:
            ...
            try:
                ...
                session_open = sensex_trade_service.is_market_session_open()
                    ...
                    if str(self._pending_entry_order_id).upper().startswith("PAPER-"):
                        ...
                        if self._pending_entry_order_id:
                            try:
                                ...
                                log_warning(f"[SensexWatch] entry timeout parse failed: {exc}")
                ...
                log_error(f"[SensexWatch] loop error: {exc}")
                self._push_event(
                    WatchEvent(
                        at=datetime.now(IST).isoformat(),
                        kind="eval_error",
    ...
    def _evaluate_sync(
        ...
        can_execute = resolve_can_execute(
            preview, plan, offhours_allowed=sensex_trade_service.allow_offhours_sensex_place()
        )
        ...
        with _lock:
            ...
            if (
                ...
                from services.sensex_order_guard import autonomous_place_allowed
                ...
                reasons = can_execute_block_errors(preview, plan, segment="sensex")
    ...
    async def _on_signal_ready(
        ...
        try:
            await broadcast_agent_update("SENSEX_WATCH_SIGNAL", payload)
        except Exception as exc:
            log_warning(f"[SensexWatch] WS broadcast failed: {exc}")
        ...
        try:
            await push_service.send_to_user(
                ...
                title=title,
                body=body[:180],
                data={"type": "sensex_watch_signal", "symbol": sym},
            )
        except Exception as exc:
            log_warning(f"[SensexWatch] push failed: {exc}")

        log_info(
            f"[SensexWatch] Checklist complete {sym} can_execute={can_place} "
    ...
    async def _try_auto_place(self, preview: Dict[str, Any], plan: Dict[str, Any]) -> None:
        async with self._get_place_lock():
            ...
            try:
                ...
                if is_kill_switch_active("sensex"):
                    self._push_event(
                        WatchEvent(
                            at=datetime.now(IST).isoformat(),
                            kind="auto_skipped",
                ...
                paper_seg = is_paper_mode_for_segment("sensex")
                ...
                if paper_seg:
                    log_info("[SensexWatch] Autonomous place in paper mode (nifty50)")

                result = await asyncio.to_thread(
                    sensex_trade_service.place_trade,
                    completed_steps=None,
                    direction=cfg.direction,
                ...
                if entry_submitted or placed:
                    ...
                    with _lock:
                        ...
                        record_gate_audit_placed("sensex", sym or "—")
                        if sym:
                            up = sym.upper()
                    ...
                    log_info(f"[SensexWatch] auto_skipped {sym} — {detail or 'unknown'}")
                ...
                log_error(f"[SensexWatch] autonomous place failed: {exc}")
                self._push_event(
                    WatchEvent(
                        at=datetime.now(IST).isoformat(),
                        kind="auto_skipped",
...
def reset_sensex_watch_placement_counters() -> None:
    reset_watch_placement_counters()


def on_sensex_trading_mode_changed(is_paper: bool) -> None:
    reset_watch_placement_counters()


def register_sensex_strategy_watch(loop: asyncio.AbstractEventLoop) -> None:
    _watch.register_loop(loop)
    st = _watch.status()
    if st.get("armed"):
        log_info(
            f"[SensexWatch] Autonomous watch active mode={st.get('mode')} dir={st.get('direction')}"
        )
