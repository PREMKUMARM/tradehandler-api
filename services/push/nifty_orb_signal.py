...
class OrbSignal:
    ...
    option_premium_risk_pts: float
    option_premium_reward_pts: float
    risk_inr: float
    reward_inr: float
    lot_size: int
    num_lots: int
...
def _build_signal(
    *,
    ...
    # Delta-based estimate of option premium move
    opt_risk_pts = risk_pts * cfg.atm_delta
    opt_reward_pts = reward_pts * cfg.atm_delta

    qty = cfg.lot_size * cfg.num_lots
    risk_inr = opt_risk_pts * qty
    reward_inr = opt_reward_pts * qty
    ...
    return OrbSignal(
        ...
        option_premium_risk_pts=opt_risk_pts,
        option_premium_reward_pts=opt_reward_pts,
        risk_inr=risk_inr,
...
def _compose_push(sig: OrbSignal) -> Dict[str, Any]:
    ...
    lines = [
        f"Entry {_fmt_level(sig.entry)}  ·  SL {_fmt_level(sig.stop_loss)}  ·  Tgt {_fmt_level(sig.target)}",
        f"Risk {sig.risk_points:.0f} pts  |  Reward {sig.reward_points:.0f} pts",
        f"OR {_fmt_level(sig.or_low)}–{_fmt_level(sig.or_high)} ({sig.or_range:.0f} pts)",
        f"Option leg (suggested, not placed): {sig.option_kind} {sig.atm_strike} (ATM)",
        (
            f"Δ≈{0.5:.2f} ⇒ ~{sig.option_premium_risk_pts:.0f} pts risk / "
            f"~{sig.option_premium_reward_pts:.0f} pts reward"
        ),
        (
            f"{sig.num_lots} lot × {sig.lot_size} qty → "
            f"Risk {_fmt_inr(sig.risk_inr)}  |  Reward {_fmt_inr(sig.reward_inr)}"
        ),
    ...
    data: Dict[str, str] = {
        ...
        "option_premium_risk_pts": f"{sig.option_premium_risk_pts:.2f}",
        "option_premium_reward_pts": f"{sig.option_premium_reward_pts:.2f}",
        "lot_size": str(sig.lot_size),
        "num_lots": str(sig.num_lots),
        "risk_inr": f"{sig.risk_inr:.2f}",
        "reward_inr": f"{sig.reward_inr:.2f}",
    }
...
async def _dispatch_push(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not user_ids:
        ...
        return {"sent": 0, "failed": 0, "reason": "no_devices", "payload": p["data"]}
